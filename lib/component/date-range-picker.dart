...
// standard was 2 i changed 0
const double _kMaxDayPickerHeight =
    _kDayPickerRowHeight * (_kMaxDayPickerRowCount + 0);
...
const double _actionFloatBannerHeight = 72;
...
class DayPicker extends StatelessWidget {
  ...
  DayPicker({
    ...
    this.showAsCalendar = false,
    this.selectableDayPredicate,
    this.restrictDates,
    this.restrictInfoCalendar,
  })  : assert(selectedFirstDate != null),
  ...
  final bool showAsCalendar;

  final List<DateTime> restrictDates;

  final Widget restrictInfoCalendar;
  ...
  List<Widget> _getDayHeaders(
    ...
    int firstDayWeek = showAsCalendar ? 1 : localizations.firstDayOfWeekIndex;
    List<String> weekDays = ['Sab', 'Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum'];

    for (int i = firstDayWeek; true; i = (i + 1) % 7) {
      final String weekday =
          showAsCalendar ? weekDays[i] : localizations.narrowWeekdays[i];
      result.add(new ExcludeSemantics(
        child: new Center(child: new Text(weekday, style: headerStyle)),
      ));
      if (i == (firstDayWeek - 1) % 7) break;
    }
          ...
          child: Column(
            ...
            children: toList(() sync* {
              yield Row(
                ...
                children: [
                  Expanded(
                    child: Text('Tidak diperbolehkan',
              ...
              if (restrictInfoCalendar != null) {
                yield restrictInfoCalendar;
              }
            }),
          )));
    } else {
  ...
  bool _containRestrictDates(List<DateTime> params) {
    assert(params?.isNotEmpty ?? false);
    assert(restrictDates?.isNotEmpty ?? false);

    List<DateTime> newRestrictDates =
        params.where((element) => element != null).toList();

    if (newRestrictDates.length == 2) {
      newRestrictDates =
          getRangeDates(newRestrictDates[0], newRestrictDates[1]);
    }

    return newRestrictDates.fold(false, (bool accu, DateTime element) {
      accu = accu ||
          restrictDates.fold(false, (bool accuChild, DateTime elementChild) {
            accuChild = accuChild ||
                (element.day == elementChild.day &&
                    element.month == elementChild.month &&
                    element.year == elementChild.year);
  ...
  Widget build(BuildContext context) {
    ...
    TextStyle weekLabelStyle = themeData.textTheme.caption;

    if (showAsCalendar) {
      weekLabelStyle = weekLabelStyle.apply(color: Style.accent);
    }

    labels.addAll(_getDayHeaders(weekLabelStyle, localizations));
        ...
        if (!disabled) {
          dayWidget = new GestureDetector(
            ...
            onTap: () {
              ...
              if (restrictDates?.isNotEmpty ?? false) {
                if (_containRestrictDates([first, last])) {
                  _showRestrictDateInfo();
                } else {
    ...
    var headerTextStyle = showAsCalendar
        ? TextStyle(fontWeight: FontWeight.w500, fontSize: 16)
        : themeData.textTheme.subtitle1;
    Widget monthTitle = new ExcludeSemantics(
      child: new Text(
        showAsCalendar
            ? formatDate(date: displayedMonth, format: 'MMMM yyyy')
    ...
    if (showAsCalendar) {
      monthTitle = Container(
          alignment: Alignment.centerLeft,
          padding: EdgeInsets.only(bottom: 24, left: 16, right: 16),
          child: monthTitle);
    } else {
      monthTitle = Container(
          height: _kDayPickerRowHeight, child: Center(child: monthTitle));
    }

    Widget gridView = GridView.custom(
      shrinkWrap: showAsCalendar,
      physics: showAsCalendar ? NeverScrollableScrollPhysics() : null,
    ...
    if (!showAsCalendar) {
      gridView = Flexible(child: gridView);
    }

    return new Padding(
      padding: const EdgeInsets.symmetric(horizontal: 8.0),
      child: new Column(
        children: [monthTitle, gridView],
      ),
    );
  }
}
...
class _MonthPickerState extends State<MonthPicker>
  ...
  void initState() {
    ...
    _dayPickerScrollController = ItemScrollController();
    _handleMonthPageChanged(monthPage);
    _updateCurrentDate();

    if (monthPage != null &&
        widget.selectedLastDate != null &&
        widget.showAsCalendar) {
      Future.delayed(Duration(milliseconds: 100)).then((_) {
        if (_dayPickerScrollController.isAttached) {
          _dayPickerScrollController.jumpTo(index: monthPage);
        }
      });
    }
  ...
  ItemScrollController _dayPickerScrollController;
  ...
  Widget _buildItems(BuildContext context, int index, int itemCount) {
    final DateTime month = _addMonthsToMonthDate(widget.firstDate, index);
    Widget dayPicker = DayPicker(
        ...
        restrictInfoCalendar: widget.restrictInfoCalendar,
        showAsCalendar: widget.showAsCalendar);

    double bottom = widget.selectedFirstDate != null && itemCount - 1 == index
        ? _actionFloatBannerHeight + 16
        : itemCount - 1 != index ? 16 : 0;

    dayPicker = Padding(
        child: dayPicker,
        padding: EdgeInsets.only(bottom: bottom, top: index == 0 ? 16 : 0));
  ...
  Widget build(BuildContext context) {
    return new SizedBox(
      width: widget.showAsCalendar ? Get.width : _kMonthPickerPortraitWidth,
      height: widget.showAsCalendar ? Get.height : _kMaxDayPickerHeight,
      child: new Stack(
        children: toList(() sync* {
          ValueKey<DateTime> valueKey = ValueKey<DateTime>(
              widget.selectedFirstDate == null
                  ? widget.selectedFirstDate
                  : widget.selectedLastDate);
          int itemCount = _monthDelta(widget.firstDate, widget.lastDate) + 1;

          Widget scrollContent;

          if (widget.showAsCalendar) {
            scrollContent = ScrollablePositionedList.builder(
              itemScrollController: _dayPickerScrollController,
              scrollDirection: widget.scrollDirection,
              itemCount: itemCount,
              minCacheExtent: 2,
              itemBuilder: (context, i) => _buildItems(context, i, itemCount),
            );
          } else {
            scrollContent = NotificationListener<ScrollStartNotification>(
              onNotification: (_) {
              ...
              child: NotificationListener<ScrollEndNotification>(
                ...
                child: new PageView.builder(
                  ...
                  itemBuilder: (context, i) =>
                      _buildItems(context, i, itemCount),
          ...
          /// arrows
          if (!widget.showAsCalendar) {
            yield new PositionedDirectional(
              top: 0.0,
              start: 8.0,
              child: new Semantics(
...
class _DatePickerDialogState extends State<DatePickerDialog> {
  ...
  void _handleCancel() {
    if (widget.onCancelSelected != null) {
  ...
  void _handleOk() {
    ...
    if (widget.onSelectedDates != null) {
      widget.onSelectedDates(result);
    } else {
    ...
    switch (_mode) {
      case DatePickerMode.day:
        return new MonthPicker(
          key: _pickerKey,
          scrollDirection:
              widget.showAsCalendar ? Axis.vertical : Axis.horizontal,
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.showAsCalendar && _selectedFirstDate != null) {
      actions = Container(
        width: Get.width - 32,
        height: _actionFloatBannerHeight,
          ...
          children: <Widget>[
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: toList(() sync* {
                  String date = formatDate(date: _selectedFirstDate);
                  if (_selectedLastDate != null) {
                    date += ' - ${formatDate(date: _selectedLastDate)}';
                  }

                  yield Text("Choose date",
    ...
    Widget widgetAsDialog;

    if (widget.showAsCalendar) {
      widgetAsDialog = picker;

      if (actions != null) {
        widgetAsDialog = Stack(
            alignment: Alignment.bottomCenter,
            children: [widgetAsDialog, actions]);
      }
    } else {
      Widget header(orientation) {
        return new _DatePickerHeader(
          selectedFirstDate: _selectedFirstDate,
          selectedLastDate: _selectedLastDate,
          mode: _mode,
      ...
      List<Widget> childrenWidgetDialog = [picker, actions];

      widgetAsDialog = new Dialog(child: new OrientationBuilder(
          ...
          case Orientation.portrait:
            return new SizedBox(
              ...
              child: new Column(
                ...
                children: toList(() sync* {
                  yield header(orientation);
                  yield new Container(
                    color: theme.dialogBackgroundColor,
                    child: new Column(
                      mainAxisSize: MainAxisSize.min,
                      crossAxisAlignment: CrossAxisAlignment.stretch,
                      children: childrenWidgetDialog,
                    ),
                  );
                }),
              ),
    ...
    if (widget.showAsCalendar) {
      return Material(child: widgetAsDialog);
    }
...
Future<List<DateTime>> showDatePicker({
  ...
  List<DateTime> restrictDates,
  Locale locale,
  Widget restrictInfoCalendar,
  TextDirection textDirection,
}) async {
  ...
  Widget child = new DatePickerDialog(
      ...
      restrictInfoCalendar: restrictInfoCalendar);
